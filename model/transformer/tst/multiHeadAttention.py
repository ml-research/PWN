...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self,
        ...
        linear_layer = nn.Linear if not self.native_complex else ComplexLinear

        # Query, keys and value matrices
        self._W_q = linear_layer(d_model, q*self._h)
        self._W_k = linear_layer(d_model, q*self._h)
        self._W_v = linear_layer(d_model, v*self._h)

        # Output linear function
        self._W_o = linear_layer(self._h*v, d_model)
    ...
    def forward(self,
        ...
        if mask == "subsequent":
            ...
            if self.native_complex:
                self._scores = torch.complex(self._scores.real.masked_fill(future_mask, float('-inf')),
                                             self._scores.imag.masked_fill(future_mask, float('-inf')))
            else:
                self._scores = self._scores.masked_fill(future_mask, float('-inf'))

        if self.native_complex:
            # Apply sotfmax
            self._scores = complex_softmax(self._scores)

        else:
            # Apply sotfmax
            self._scores = F.softmax(self._scores, dim=-1)

        # scores_min = self._scores.min()
        # self._scores = (self._scores - scores_min) / (self._scores.max() - scores_min)
