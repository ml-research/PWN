"""
From https://github.com/philipperemy/n-beats
"""
...
class NBeatsNet(nn.Module):
    ...
    def compile(self, loss: str, optimizer: Union[str, Optimizer], lr=1e-4):
        if loss == 'mae':
            loss_ = l1_loss
        elif loss == 'mse':
            loss_ = mse_loss
            ...
            loss_ = lambda out, label: 2 * (torch.abs(out - label) /
        ...
        if isinstance(optimizer, str):
            ...
            opt_ = opt_(lr=lr, params=self.parameters())
        else:
            opt_ = optimizer
        self._opt = opt_
        self._loss = loss_
    ...
    def fit(self, x_train, y_train, validation_data=None, epochs=10, batch_size=32):
        ...
        for epoch in range(epochs):
            ...
            if validation_data is not None:
                ...
                x_test, y_test = torch.tensor(x_test, dtype=torch.float).to(self.device), \
                                 torch.tensor(y_test, dtype=torch.float).to(self.device)
                self.eval()
                _, forecast = self(x_test)
                test_loss = self._loss(forecast, squeeze_last_dim(y_test)).item()
