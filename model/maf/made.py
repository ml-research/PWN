...
class MADE(nn.Module):
    def __init__(
        ...
        for i in range(len(dim_list) - 2):
            self.layers.append(MaskedLinear(dim_list[i], dim_list[i + 1]).to("cuda"),)
            self.layers.append(ReLU())
        # Hidden layer to output layer.
        self.layers.append(MaskedLinear(dim_list[-2], dim_list[-1]).to("cuda"))
        # Create model.
        self.model = nn.Sequential(*self.layers).to("cuda")
    ...
    def _create_masks(self) -> None:
        ...
        for module in self.model.modules():
            if isinstance(module, MaskedLinear):
                module.initialise_mask(next(mask_iter).to('cuda'))
