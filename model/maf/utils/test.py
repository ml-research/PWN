...
def test_maf(model, test_loader):
    model.eval()
    test_loss = []
    ...
    with torch.no_grad():
        for batch in test_loader:
            u, log_det = model.forward(batch[0].to("cuda").float())

            log_dets.append(log_det)
            negloglik_loss = 0.5 * (u ** 2).sum(dim=1)
            negloglik_loss += 0.5 * batch[0].shape[1] * np.log(2 * math.pi)
